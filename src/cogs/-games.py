...
class Games(commands.Cog):
    ...
    async def flip(self,ctx,*,bet='1'):
        if bet == 'one' or bet == '1️⃣':
            await ctx.send("You're trying my patience, obviously 1 is a number, but not like this, not like this...")
            return
        try:
            winnings = int(bet)*2
        except ValueError:
        ...
        try:
            ...
            player = None
        if winnings is 0:
            await ctx.send("Bet can't be 0 coins")
            return
        if player is None:
            await ctx.send("You don't have a character profile yet. Create one with <prefix>create in order to play")
            return
